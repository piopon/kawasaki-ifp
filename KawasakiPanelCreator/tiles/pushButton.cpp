 ...
 */
__fastcall pushButton::pushButton(TComponent *myOwner, tileType typeOfTile) : zeroTile(myOwner,typeOfTile)
{
	ID = 2;
	type = typeOfTile;
...
//destruktor
//---------------------------------------------------------------------------
__fastcall pushButton::~pushButton()
{
	/************************************
 ...
 * arg: myOwner -
 */
zeroTile* pushButton::createTile(TComponent *myOwner, tileType typeOfTile)
{
	return new pushButton(myOwner,typeOfTile);
}
//---------------------------------------------------------------------------
/* metoda do wyswietlania okienka edycyjnego danego kafelka
 */
zeroTile* pushButton::showEditor(void)
{
 ...
 */
void pushButton::setTileContent(void)
{
	try {
 ...
 */
void pushButton::setTileContent(zeroTile *originalTile)
{
	if (originalTile && originalTile->ClassNameIs("pushButton")) {
 ...
 */
void pushButton::getTileContent(void)
{
	try {
 ...
 */
void pushButton::saveToXML(TXMLDocument *xmlSource, int tileNo)
{
 ...
 */
void pushButton::loadFromXML(TXMLDocument *xmlSource, int tileNo)
{
	//sprawdzamy istnieje zrodlo XML
	if (xmlSource) {
		IXMLNode *nodeRoot, *nodeTile, *nodeParams;
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
AnsiString pushButton::saveToIFP(int tileNo)
{
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
void pushButton::loadFromIFP(String ifpData, int tileNo)
{
	String bcgColor[2], labelColor, temp, labelsText[4];
    int labelStart, labelStop, sigNo, opCond;
	temp = ifpData;
