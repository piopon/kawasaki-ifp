 ...
 */
__fastcall pilotLamp::pilotLamp(TComponent *myOwner, tileType typeOfTile) : zeroTile(myOwner,typeOfTile)
{
	ID = 1;
	type = typeOfTile;
	//kolor lampki kafelka w trybie ON
	lampOn = new TCircle(this);
	lampOn->Parent = this;
	lampOn->HitTest = false;
	lampOn->Height = Height*0.75;
	lampOn->Width = Width*0.75;
	lampOn->Position->X = (Width-lampOn->Width)/2;
	lampOn->Position->Y = (Height-lampOn->Height)/2+10;
	lampOn->Fill->Color = gray;
	//kolor lampki kafelka w trybie OFF
	lampOff = new TCircle(this);
	lampOff->Parent = this;
	lampOff->HitTest = false;
	lampOff->Height = Height*0.75;
	lampOff->Width = Width*0.75;
	lampOff->Position->X = (Width-lampOn->Width)/2;
	lampOff->Position->Y = (Height-lampOn->Height)/2+10;
	lampOff->Fill->Color = gray;
	//tlo wlaczone musi byc domyslnie schowane a wyl na wierzchu
	lampOn->SendToBack();
	lampOff->BringToFront();
	...
	//znak wykrzyknika
	exclamation = new TLabel(this);
	exclamation->Parent = this;
	exclamation->Height = 50;
	exclamation->Width = 20;
	exclamation->Position->X = Width-exclamation->Width-10;
	exclamation->Position->Y = Height-exclamation->Height;
	exclamation->Text = "!";
	exclamation->Font->Size = 40;
	exclamation->Font->Style << fsBold;
	exclamation->StyleLookup = "exclamationMarkStyle1";
...
//destruktor
//---------------------------------------------------------------------------
__fastcall pilotLamp::~pilotLamp()
{
	/************************************
 ...
 * arg: myOwner -
 */
zeroTile* pilotLamp::createTile(TComponent *myOwner, tileType typeOfTile)
{
	return new pilotLamp(myOwner,typeOfTile);
}
//---------------------------------------------------------------------------
/* metoda do wyswietlania okienka edycyjnego danego kafelka
 */
zeroTile* pilotLamp::showEditor(void)
{
 ...
 */
void pilotLamp::setTileContent(void)
{
	try {
		//uzupelniamy wyglad kafelka
		background_altOff->Fill->Color = tilePropertiesWindow->colorButtonBackg_altOff->Color;
		lampOn->Fill->Color = tilePropertiesWindow->colorButtonLampOn->Color;
		lampOff->Fill->Color = tilePropertiesWindow->colorButtonLampOff->Color;
		...
		signalNo = tilePropertiesWindow->editLampSignal->Text.ToInt();
		exclMark = tilePropertiesWindow->checkBoxExclMark->IsChecked;
		exclamation->Visible = exclMark;
	} catch (...) {
 ...
 */
void pilotLamp::setTileContent(zeroTile *originalTile)
{
	if (originalTile && originalTile->ClassNameIs("pilotLamp")) {
 ...
 */
void pilotLamp::getTileContent(void)
{
	try {
		//pobieramy wyglad kafelka
		tilePropertiesWindow->colorButtonBackg_altOff->Color = background_altOff->Fill->Color;
		tilePropertiesWindow->colorButtonLampOn->Color = lampOn->Fill->Color;
		tilePropertiesWindow->colorButtonLampOff->Color = lampOff->Fill->Color;
		...
		tilePropertiesWindow->editLampSignal->Text = IntToStr(signalNo);
		tilePropertiesWindow->checkBoxExclMark->IsChecked = exclMark;
	} catch (...) {
	}
 ...
 */
void pilotLamp::saveToXML(TXMLDocument *xmlSource, int tileNo)
{
	...
	if (xmlSource) {
		...
		tileColors bcgColor,lampColor,lblColor;
		...
		nodeTile->SetAttribute("excl",exclMark);
		//dodajemy kolor tla
		nodeParams = nodeTile->AddChild("backg",0);
		bcgColor = static_cast<tileColors>(background_altOff->Fill->Color);
		nodeParams->SetAttribute("color",colorToStr(bcgColor));
		//dodajemy kolor lampki
		nodeParams = nodeTile->AddChild("lamp",1);
		lampColor = static_cast<tileColors>(lampOn->Fill->Color);
		nodeParams->SetAttribute("on",colorToStr(lampColor));
		lampColor = static_cast<tileColors>(lampOff->Fill->Color);
		nodeParams->SetAttribute("off",colorToStr(lampColor));
 ...
 */
void pilotLamp::loadFromXML(TXMLDocument *xmlSource, int tileNo)
{
	//sprawdzamy istnieje zrodlo XML
	if (xmlSource) {
		IXMLNode *nodeRoot, *nodeTile, *nodeParams;
		...
		if (nodeRoot->HasChildNodes && nodeRoot->ChildNodes->Count>=tileNo+1) {
			...
			if ((int)nodeTile->GetAttribute("ID")==ID) {
				...
				exclMark = nodeTile->GetAttribute("excl");
				exclamation->Visible = exclMark;
                String bcgColor, lampColor, labelColor;
				//odczytujemy informacje o kolorze tla kafelka
				nodeParams = nodeTile->ChildNodes->Get(0);
				bcgColor = nodeParams->GetAttribute("color");
				background_altOff->Fill->Color = strToColor(bcgColor);
				//odczytujemy informacje o kolorze lampki kafelka
				nodeParams = nodeTile->ChildNodes->Get(1);
				lampColor = nodeParams->GetAttribute("on");
				lampOn->Fill->Color = strToColor(lampColor);
				lampColor = nodeParams->GetAttribute("off");
				lampOff->Fill->Color = strToColor(lampColor);
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
AnsiString pilotLamp::saveToIFP(int tileNo)
{
	...
	result += IntToStr(colorToInt(currColor))+",";
	//kolor gdy lampka ON
	currColor = static_cast<tileColors>(lampOn->Fill->Color);
	result += IntToStr(colorToInt(currColor))+",";
	//kolor gdy lampka OFF
	currColor = static_cast<tileColors>(lampOff->Fill->Color);
	result += IntToStr(colorToInt(currColor))+",";
	//numer sygnalu
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
void pilotLamp::loadFromIFP(String ifpData, int tileNo)
{
	String bcgColor, lampColor[2], labelColor, temp, labelsText[4];
    int labelStart, labelStop, sigNo, exclM;
	temp = ifpData;
	...
	temp = temp.SubString(labelStop+1,temp.Length()+1-(labelStop+1));
	labelStop = temp.Pos(",");
	lampColor[0] = temp.SubString(0,labelStop-1);
	//odczytujemy informacje o kolorze tla OFF
	temp = temp.SubString(labelStop+1,temp.Length()+1-(labelStop+1));
	labelStop = temp.Pos(",");
	lampColor[1] = temp.SubString(0,labelStop-1);
	...
	temp = temp.SubString(labelStop+1,temp.Length()+1-(labelStop+1));
	exclM = StrToInt(temp.SubString(0,1));
	/********* UAKTUALNIAMY KAFELEK *********/
	signalNo = sigNo;
	exclMark = exclM;
	exclamation->Visible = exclM;
	background_altOff->Fill->Color = intToColor(StrToInt(bcgColor));
	lampOn->Fill->Color = intToColor(StrToInt(lampColor[0]));
	lampOff->Fill->Color = intToColor(StrToInt(lampColor[1]));
