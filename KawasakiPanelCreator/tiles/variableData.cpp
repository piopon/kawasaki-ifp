 ...
 */
__fastcall variableData::variableData(TComponent *myOwner, tileType typeOfTile) : zeroTile(myOwner,typeOfTile)
{
	ID = 8;
	type = typeOfTile;
...
//destruktor
//---------------------------------------------------------------------------
__fastcall variableData::~variableData()
{
	/************************************
 ...
 * arg: myOwner -
 */
zeroTile* variableData::createTile(TComponent *myOwner, tileType typeOfTile)
{
	return new variableData(myOwner,typeOfTile);
}
//---------------------------------------------------------------------------
/* metoda do wyswietlania okienka edycyjnego danego kafelka
 */
zeroTile* variableData::showEditor(void)
{
 ...
 */
void variableData::setTileContent(void)
{
	try {
		...
		intDataType = tilePropertiesWindow->dataTypeINT->IsChecked;
	} catch (...) {
	}
}
//---------------------------------------------------------------------------
 ...
 */
void variableData::setTileContent(zeroTile *originalTile)
{
	if (originalTile && originalTile->ClassNameIs("variableData")) {
 ...
 */
void variableData::getTileContent(void)
{
	try {
		...
		if (intDataType) {
			tilePropertiesWindow->dataTypeINT->IsChecked = true;
			tilePropertiesWindow->dataTypeREAL->IsChecked = false;
		} else {
			tilePropertiesWindow->dataTypeINT->IsChecked = false;
			tilePropertiesWindow->dataTypeREAL->IsChecked = true;
		}
	} catch (...) {
	}
}
 ...
 */
void variableData::saveToXML(TXMLDocument *xmlSource, int tileNo)
{
	...
	if (xmlSource) {
		...
		nodeTile->SetAttribute("int",intDataType);
		//dodajemy informacje o sygnale
		nodeParams = nodeTile->AddChild("variable",0);
		nodeParams->SetAttribute("name",variable);
		nodeParams->SetAttribute("figNo",figuresNo);
 ...
 */
void variableData::loadFromXML(TXMLDocument *xmlSource, int tileNo)
{
	//sprawdzamy istnieje zrodlo XML
	if (xmlSource) {
		IXMLNode *nodeRoot, *nodeTile, *nodeParams;
		...
		if (nodeRoot->HasChildNodes && nodeRoot->ChildNodes->Count>=tileNo+1) {
			...
			if ((int)nodeTile->GetAttribute("ID")==ID) {
				...
				intDataType = nodeTile->GetAttribute("int");
				//odczytujemy informacje zmiennych wewnetrznych kafelka
				nodeParams = nodeTile->ChildNodes->Get(0);
				variable = nodeParams->GetAttribute("name");
				figuresNo = nodeParams->GetAttribute("figNo");
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
AnsiString variableData::saveToIFP(int tileNo)
{
 ...
 * arg: fileName - nazwa pliku do ktorego zapisujemy
 */
void variableData::loadFromIFP(String ifpData, int tileNo)
{
	String bcgColor, labelColor, temp, labelsText[2], var;
	int labelStart, labelStop, figNo, oper, intD;
	temp = ifpData;
	...
	//odczytujemy informacje o sposobie wyswietlania liczby (INT / REAL)
	temp = temp.SubString(labelStop+1,temp.Length()+1-(labelStop+1));
	labelStop = temp.Pos(",");
	intD = StrToInt(temp.SubString(0,labelStop-1));
